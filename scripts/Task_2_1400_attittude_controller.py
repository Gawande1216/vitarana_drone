...
class Edrone():
    ...
    def roll_set_pid(self, roll):
        self.Kp[0] = roll.Kp * 0.1  # This is just for an example. You can change the ratio/fraction value accordingly
    ...
    def pid(self):
        ...
        self.throttle = (self.setpoint_cmd[3] - 1000) * 1024 / 1000
        ...
        # error sum
       	self.error_sum[0] = (self.error_sum[0] + error[0])
        self.error_sum[1] = (self.error_sum[1] + error[1])
        self.error_sum[2] = (self.error_sum[2] + error[2])

        #d_error
        d_error = [0,0,0,0]
       	d_error[0] = (error[0] - self.prev_error[0])
        d_error[1] = (error[1] - self.prev_error[1])
        d_error[2] = (error[2] - self.prev_error[2])
        ...
        yaw_out = self.Kp[2] * error[2] + self.Ki[2] * self.error_sum[2] + self.Kd[2] * d_error[2]
        
       	self.prev_error[0] = error[0]
        self.prev_error[1] = error[1]
        self.prev_error[2] = error[2]
        
       	self.pwm_cmd.prop1 = (self.throttle - roll_out + pitch_out) # - yaw_out
       	self.pwm_cmd.prop2 = (self.throttle - roll_out - pitch_out) # + yaw_out
       	self.pwm_cmd.prop3 = (self.throttle + roll_out - pitch_out) # - yaw_out
       	self.pwm_cmd.prop4 = (self.throttle + roll_out + pitch_out) # + yaw_out
